...
export async function middleware(request: NextRequest) {
  ...
  if (pathname === "/") {
    // Check for authentication token for redirect logic
    const token =
  ...
  if (!payload) {
    ...
    if (refreshToken) {
      ...
      if (refreshPayload) {
        ...
        const isLongLived = !!(
          refreshPayload.exp &&
          refreshPayload.exp - refreshPayload.iat! > 100 * 24 * 60 * 60
        ); // More than 100 days means remember me was used
        const {
          accessToken,
  ...
  if (
    !welcomeRequiredPaths.some((path) => pathname.startsWith(path)) &&
