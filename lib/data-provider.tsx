...
export function DataProvider({
  ...
  children: ReactNode;
  initialData?: { monthData: any[]; yearData: any[]; settings?: any };
}) {
  ...
  const [currentMonthData, setCurrentMonthData] = useState<JourneyData>(
    initial.currentMonthData,
  );
  ...
  const fetchAllData = async (silent = false) => {
    try {
      ...
      const [monthResponse, yearResponse, settingsResponse] = await Promise.all(
        [
          fetch(
            `/api/progress-range?startDate=${year}-${String(month).padStart(2, "0")}-01&endDate=${year}-${String(month).padStart(2, "0")}-31`,
          ),
          fetch(
            `/api/progress-range?startDate=${year}-01-01&endDate=${year}-12-31`,
          ),
          fetch("/api/settings"),
        ],
      );
      ...
      localStorage.setItem(
        "progressCache",
        JSON.stringify(transformedYearData),
      );
    } catch (error) {
