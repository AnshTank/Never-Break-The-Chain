...
// FIXED: Pre-generate particle data to avoid hydration mismatch
const generateParticles = (count: number, seed: number) => {
  const particles = [];
  for (let i = 0; i < count; i++) {
    // Use seed-based pseudo-random for consistent server/client rendering
    ...
    particles.push({
      ...
      startY: 30 + pseudoRandom(i * 5) * 40,
      delay: pseudoRandom(i * 7) * 5,
      duration: 4 + pseudoRandom(i * 11) * 3,
      size: 1.5 + pseudoRandom(i * 13) * 2.5
    });
  }
  return particles;
};
...
const generateBubbles = (count: number, seed: number) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    bubbles.push({
      ...
      delay: pseudoRandom(i * 4) * 3,
      duration: 4 + pseudoRandom(i * 6) * 3,
      size: 40 + pseudoRandom(i * 8) * 80
    });
  }
  return bubbles;
};
...
// Bubble Effect Component - FIXED for SSR
...
const WelcomePage = () => {
  ...
  // Complete phase and unlock next - MANUAL PROGRESSION
        ...
        scrollTimeout = setTimeout(() => {
          ...
          if (currentElement) {
            ...
            if (now - lastScrollTime > 3000) {
              toast.info('🔒 Click the button below to continue!', {
                duration: 3000,
                position: 'top-center',
                style: {
                  ...
                  <div>
                    ...
                    <div className="flex items-center bg-white/5 rounded-lg overflow-hidden">
                      ...
                      <div className="flex-1 text-center">
                        <span className="text-white text-xl font-bold">{config.minMinutes}</span>
            ...
            >
              ...
              <div className="space-y-3 md:space-y-4">
                <div>
                  ...
                  <input
                    type="email"
                    value="user@neverbreakthechain.com"
                    disabled
                    className="w-full px-3 md:px-4 py-2.5 md:py-3 bg-white/5 border border-white/10 rounded-lg md:rounded-xl text-white/60 text-sm md:text-base cursor-not-allowed"
                  />
                </div>
