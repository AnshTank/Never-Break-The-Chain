...
export default function DayCell({
  ...
  const getHourColor = () => {
    if (isFuture)
      return "bg-gradient-to-br from-gray-100 to-gray-200 border border-gray-300 opacity-50";
    if (isToday)
      return "bg-gradient-to-br from-blue-200 to-blue-300 border-2 border-blue-400";

    // For past days only, use 5-color system based on task completion
    if (!entry)
      return "bg-gradient-to-br from-red-100 to-red-200 border border-red-300"; // 0 tasks - red

    if (completedCount === 0)
      return "bg-gradient-to-br from-red-100 to-red-200 border border-red-300"; // 0 tasks - red
    if (completedCount === 1)
      return "bg-gradient-to-br from-orange-100 to-orange-200 border border-orange-300"; // 1 task - orange
    if (completedCount === 2)
      return "bg-gradient-to-br from-yellow-100 to-yellow-200 border border-yellow-300"; // 2 tasks - yellow
    if (completedCount === 3)
      return "bg-gradient-to-br from-blue-100 to-blue-200 border border-blue-300"; // 3 tasks - blue
    if (completedCount === 4)
      return "bg-gradient-to-br from-green-100 to-green-200 border border-green-300"; // 4 tasks - green
      ...
      <div
        ...
        title={
          isFuture
            ...
            : entry
              ? `${completedCount}/4 tasks, ${hours.toFixed(1)}h`
              : "Click to add entry"
        }
      >
        <div
          className={`text-xs sm:text-sm font-bold ${isFuture ? "text-gray-400" : isToday ? "text-blue-900" : "text-gray-900"}`}
        >
          {day}
        </div>
        {!isFuture && (
          <div
            className={`text-xs font-semibold ${isToday ? "text-blue-800" : "text-gray-700"}`}
          >
            {completedCount}/4
          </div>
        )}
        ...
        {isToday && (
          <div className="text-xs text-blue-800 font-bold hidden sm:block">
            TODAY
          </div>
        )}
        {isFuture && (
