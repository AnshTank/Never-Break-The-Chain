...
export default function DayCell({ day, date, entry, isToday, onEntryChange }: DayCellProps) {
  ...
  const completedCount = entry?.tasks ? entry.tasks.filter((t) => t.completed).length : 0
  ...
  const handleSave = (newEntry: DayEntry) => {
  ...
  return (
    <>
      <div
        onClick={() => !isFuture && setShowModal(true)}
        className={`
      ...
      >
        ...
        {entry && !isFuture && <div className={`text-xs font-semibold ${isToday ? "text-blue-800" : "text-gray-700"}`}>{completedCount}/4</div>}
        {allCompleted && !isFuture && <div className="text-sm sm:text-lg">✓</div>}
