...
export default function TimerPage() {
  ...
  // Initialize MNZD tasks
  useEffect(() => {
    const mnzdTaskList = mnzdTasks.map((task) => ({
      id: task.id,
      name: task.name,
      sessions: 0,
      totalTime: 0,
  ...
  useEffect(() => {
    ...
    if (savedData) {
      ...
      setCustomAccentColor(data.customAccentColor || "#3b82f6");
    }
  ...
  const handleComplete = () => {
    ...
    if (currentTask && mode === "focus") {
      ...
      if (mnzdTasks.find((t) => t.id === currentTask)) {
    ...
    if (mode === "focus") {
      ...
      setTimeout(() => {
        setMode("break");
        setSelectedDuration(sessionSettings.breakTime);
        setTimeLeft(sessionSettings.breakTime * 60);
        if (autoStart) {
          setIsRunning(true);
        }
      }, 2000);
      ...
      setTimeout(() => {
        setMode("focus");
        setSelectedDuration(sessionSettings.focusTime);
        setTimeLeft(sessionSettings.focusTime * 60);
        if (autoStart) {
          setIsRunning(true);
        }
      }, 2000);
    ...
    >
      ...
      {isMobile && (
        <div
          ...
          style={{
            background: `linear-gradient(135deg, ${theme.accent}10 0%, ${theme.accent}05 50%, transparent 100%)`,
      ...
      <div className="absolute top-0 left-0 right-0 z-30 flex items-center justify-between p-4 md:p-8">
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <button
            ...
            className={`px-4 py-2 md:px-8 md:py-3 rounded-full ${theme.panel} backdrop-blur-xl transition-all duration-500 ${theme.border} border hover:scale-105 shadow-2xl`}
            style={{ color: theme.accent }}
          >
      ...
      <div className="flex items-center justify-center min-h-screen p-8">
        <div className="text-center">
          <div className="mb-12">
            <h1
              className={`text-8xl font-extralight ${theme.text} mb-6 transition-all duration-700 tracking-wide`}
            >
              {mode === "focus" ? "Focus Time" : "Break Time"}
            </h1>
            <p
              className={`${theme.text} opacity-90 text-2xl font-light max-w-4xl mx-auto leading-relaxed transition-all duration-700`}
            >
              {currentQuote}
            </p>
          </div>
          ...
          {currentTask && mode === "focus" && (
            <div
              className={`mb-12 inline-block px-8 py-4 rounded-full ${theme.panel} backdrop-blur-xl ${theme.border} border transition-all duration-700 shadow-xl`}
            >
          ...
          <div className="relative mb-16">
            ...
            <svg
              className="w-96 h-96 mx-auto transform -rotate-90 relative z-10"
              viewBox="0 0 100 100"
            >
              <circle
                cx="50"
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                <div
                  className={`text-7xl font-extralight ${theme.text} mb-4 tabular-nums transition-all duration-700 tracking-wider`}
                >
                  {formatTime(timeLeft)}
                </div>
                <div
                  className={`${theme.text} opacity-70 text-xl font-light transition-all duration-700`}
                >
                  {isRunning
                    ? mode === "focus"
                      ? "Deep Focus"
                ...
                {isRunning && mode === "focus" && (
                  <div
                    className={`mt-3 text-lg ${theme.text} opacity-50 transition-all duration-700`}
                  >
          ...
          <div className="flex justify-center gap-8 mb-12">
            {!isRunning ? (
              <button
                onClick={start}
                className={`px-16 py-5 rounded-full ${theme.panel} backdrop-blur-xl transition-all duration-500 shadow-2xl hover:shadow-3xl hover:scale-110 ${theme.border} border group`}
              >
                <span
                  className={`text-2xl font-medium ${theme.text} group-hover:scale-105 transition-transform duration-300`}
                >
                  Start
                </span>
              </button>
            ...
            <button
              onClick={reset}
              className={`px-12 py-5 rounded-full ${theme.panel} opacity-80 backdrop-blur-xl transition-all duration-500 hover:scale-110 ${theme.border} border group`}
            >
              <span
                className={`text-xl font-medium ${theme.text} group-hover:scale-105 transition-transform duration-300`}
              >
                Reset
              </span>
            </button>
          ...
          {/* Duration Selector */}
          <div
            className={`inline-flex ${theme.panel} backdrop-blur-xl rounded-full p-3 shadow-2xl ${theme.border} border`}
          >
      ...
      <div
        className={`fixed inset-y-0 right-0 w-96 ${
          theme.panel
      ...
      >
        <div className="flex flex-col h-full">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {mnzdTasks.map((task) => {
                      const progress =
                        stats.mnzdProgress[
                          task.id as keyof typeof stats.mnzdProgress
                        ];
                      const isComplete = progress >= task.minMinutes;
                      const progressPercent = Math.min(
                        (progress / task.minMinutes) * 100,
                        100
                      );
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between mb-3">
                            ...
                            >
                              {currentTask === task.id ? "Active" : "Select"}
                            </button>
                          </div>
                        ...
                        >
                          <div className="absolute inset-0 opacity-10">
                            <div
                              className="w-full h-full bg-gradient-to-br"
                              style={{
                                background: `linear-gradient(135deg, ${task.color}40 0%, transparent 100%)`,
                              }}
                            ></div>
                          </div>
                          <div className="relative z-10">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    {backgroundSounds.map((sound) => (
                      <button
                        key={sound.name}
                        onClick={() => setBackgroundSound(sound.file)}
                        className={`w-full p-3 rounded-lg text-left transition-all duration-500 hover:scale-105 transform ${
          ...
          {/* Panel Navigation - Fixed at bottom */}
          <div
            ...
            >
              {[
                ...
                <button
                  ...
                  style={{
                    backgroundColor:
                      activePanel === item.id ? theme.accent : "transparent",
                  }}
                >
