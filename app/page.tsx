...
import { useUserStatus } from "@/hooks/use-data"

export default function Home() {
  const router = useRouter()
  const [isLoading, setIsLoading] = useState(true)
  const [activeTab, setActiveTab] = useState<"calendar" | "progress">("calendar")
  const { isNewUser, loading: userLoading } = useUserStatus()
  ...
  // Single auth check with early return
  useEffect(() => {
    let mounted = true
  ...
  // Handle new user redirect with dependency optimization
  useEffect(() => {
    if (!userLoading && isNewUser === true) {
      router.push('/welcome')
    }
  }, [isNewUser, userLoading, router])
